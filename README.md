#  System Online | Miqueias Barros Portfolio

![Project Banner](https://raw.githubusercontent.com/MiqueiasFBarros/personal-portfolio/refs/heads/main/assets/images/logo.png)

> **Status:** System Online   
> **Theme:** Cyberpunk / Terminal / Neon

##  Sobre

Bem-vindo ao meu hub digital. Este portfólio não é apenas uma vitrine de projetos, mas uma demonstração de habilidades em **Front-End**, **UI Design** e **Interatividade**.

Inspirado na estética Cyberpunk e em interfaces de terminais retrô, o site busca oferecer uma experiência imersiva através de efeitos visuais (CRT, Glitch, Matrix) e interações sonoras, mantendo a performance e a acessibilidade.

> "Desenvolvedor Front-End especialista em dar vida a layouts complexos. Com um foco obsessivo em interatividade e Interfaces Imersivas, transformo conceitos visuais em experiências digitais dinâmicas. De Alagoas para o mundo."

---

##  Funcionalidades & Efeitos Visuais

Este projeto foi desenvolvido "Vanilla" (sem frameworks pesados) para garantir máximo controle sobre as animações e performance.

- ** CRT & Scanline Overlay:** Simulação de monitores de tubo antigos usando CSS puro.
- ** Matrix Rain:** Efeito clássico de chuva de código renderizado em HTML5 Canvas.
- ** Terminal Loader:** Tela de "BIOS Boot" simulada com JavaScript na inicialização.
- ** Custom Cursor:** Cursor personalizado com rastro (trail) e efeitos de hover.
- ** Sound FX:** Feedback sonoro ao interagir com botões e links (Sound Design).
- ** Easter Eggs:** Interações escondidas (ex: Personagem da Twitch no rodapé).
- ** Responsividade:** Layout adaptável para Mobile, Tablet e Desktop.

---

## 🛠️ Tecnologias Utilizadas

![HTML5](https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/css3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E)
![Figma](https://img.shields.io/badge/figma-%23F24E1E.svg?style=for-the-badge&logo=figma&logoColor=white)

- **Fontes:** JetBrains Mono (Coding aesthetic) & Inter.
- **Ícones:** Phosphor Icons.
- **Animações:** CSS Keyframes & JS Vanilla.

---

## Projetos em Destaque

O portfólio apresenta meus principais trabalhos, incluindo:

| Projeto                     | Descrição                                                        | Tech Stack                   |
| :-------------------------- | :--------------------------------------------------------------- | :--------------------------- |
| **Portal Destino Coruripe** | Portal turístico completo com reservas e guia local. (Freelance) | HTML, CSS, JS, SEO           |
| **Ping-Pong Arcade**        | Recriação do clássico jogo com física de colisão.                | Canvas API, JS Game Loop     |
| **Golden Dice**             | App Android para jogadores de RPG com dados e histórico offline. | Java, SQLite, Android Studio |
| **PokeEco**                 | Simulador de ecossistema autônomo com IA simples.                | Java, POO Avançada           |

---

##  Como Executar Localmente

1. **Clone o repositório:**
   ```bash
   git clone [https://github.com/MiqueiasFBarros/Portfolio-System.git](https://github.com/MiqueiasFBarros/Portfolio-System.git)
   ```


